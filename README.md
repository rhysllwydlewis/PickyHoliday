...
The owner admin dashboard is available from the public **Sign in** widget and directly at `/admin/login`. The widget asks for the admin access key supplied to the owner, verifies it with the backend, and then redirects to `/admin`. Enter the server-side `ADMIN_ACCESS_TOKEN` at runtime only; the browser stores the verified token in `sessionStorage` only and never in `localStorage`. Set `ADMIN_ACCESS_TOKEN` in the server/API environment (for example Railway), and do not expose it through frontend env vars, bundled frontend code, links, defaults or prefills. For temporary non-live testing only, a test key can be configured server-side with `TEST_ADMIN_ACCESS_TOKEN` when test login is enabled; supply test credentials out of band instead of displaying them in the public frontend. Disable test login and set a real server-side `ADMIN_ACCESS_TOKEN` before launch. After login, `/admin` provides Dashboard, Enquiries, Promoted Deals, Content Pages, Site Content, Feature Flags, Operations and Settings sections. Visiting `/admin` without a session token redirects safely to `/admin/login`; logout clears the session token. Customer accounts are not live yet.
