...
PickyHoliday has pivoted from “Amadeus as the main future provider” to **Duffel as the preferred flight API path**, with **Amadeus retained as an optional sandbox/search provider**.
...
This repo is still foundation/commercial-routing work only. It does not build live booking, payments, Duffel orders, Amadeus orders, seat maps, ancillaries or booking confirmations.
...
| Affiliate/package provider | Active mock/config provider for approved partner redirect candidates. | TUI, Jet2holidays, easyJet Holidays, Loveholidays, On the Beach, Expedia-style redirects and package affiliate links. |
...
Duffel flight offers are mapped as `flight-only` enquiry results. Package provider results are mapped as `package` results with `bookingMode: 'affiliate'` only when the partner URL passes approved-domain validation.
...
`.env.example` documents mock defaults, Duffel test-mode placeholders, Amadeus optional sandbox placeholders, package/affiliate placeholders and future hotel/payment placeholders. Real credentials must be stored in deployment secrets and must never be committed. Do not add browser-side Duffel token variables.

## Staged follow-up PRs

1. Duffel provider foundation: server-side offer-request scaffold, provider registry/status and mock fixtures.
2. Affiliate/package redirect provider (current phase): approved partner config, mock/config package deals and safe redirect handling.
...
- Affiliate tracking IDs should remain server-side unless the partner explicitly approves another pattern.
- Partner redirects must only use approved domains.
