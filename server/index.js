...
const runOpsTests = async (request, includeWriteTests = false) => {
  ...
  await add('Booking.com Demand provider state', async () => { const status = registry.status(); const provider = (status.providerStatus || []).find((item) => item.provider === 'booking-demand'); return { status: provider?.enabled && !provider?.configured ? 'warn' : 'pass', message: provider ? `booking-demand is ${provider.mode}; credentials configured: ${Boolean(provider.credentialsConfigured)}.` : 'booking-demand provider was not reported.' }; });
